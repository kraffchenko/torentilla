...
namespace bencode {
  ...
  public:
    void encodeObject();
    std::vector<std::byte> encodeObject(const Value&);
    std::vector<std::byte> encodeDict(const std::map<std::string, Value>&); 
    std::vector<std::byte> encode64Int(const int64_t&);
    std::vector<std::byte> encodeString(const std::string&);
    std::vector<std::byte> encodeList(const std::vector<Value>&);
  private:
