...
: m_path{path}, m_file_name{file_name}, m_resume_file{resume_file_ref}, m_metadata{metadata}
{
  if(!std::filesystem::exists(m_path + m_file_name))
    createFile(m_metadata.getLength());
};

void torrent::File::createFile(const size_t file_size){
  std::ofstream file{m_path + m_file_name, std::ios::binary};
