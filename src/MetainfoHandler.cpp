...
std::string MetainfoHandler::returnInfoValueHash(std::map<std::string, BValue>& info_dict_ref){
  BEncode encoder{};
  std::vector<std::byte> raw_bytes {encoder.encodeDict(info_dict_ref)};

  if (std::size(raw_bytes) < 2){
    throw std::invalid_argument("Invalid argument");
  }

  std::string info_string{reinterpret_cast<char*>(raw_bytes.data()), std::size(raw_bytes)};
  boost::compute::detail::sha1 sha1{info_string};
  std::string info_string_hash{sha1};
  std::cout << info_string_hash << "\n";
  return info_string_hash;
}
